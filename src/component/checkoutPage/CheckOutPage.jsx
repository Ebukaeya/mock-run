...
import PaymentSuccess, { PaymentSuccessOverlay } from "./SuccessPayment";
...
const HardwareCheckoutPage = () => {
  ...
  const getDiscountAmount = () => {
    ...
    const discount = discounts[userSubscription] || 0;
    return (Number(selectedProduct.price.replace(/[^\d.]/g, "")) * discount) / 100;
  };
  ...
  const handleEmailSubmit = async () => {
    ...
    try {
      const endpoint = process.env.REACT_APP_Back_end_api_root + "/paystack/user/fetchUser/" + formData.email.toLowerCase();
      ...
      if (hasAccount) {
        setUserType("account");
        if (data) {
          const usertype = data.userType.userType.toLowerCase();
          let randomSubscription = null;
          console.log("User type string:", usertype);

          if (usertype.includes("starter")) {
            randomSubscription = "starter";
          } else if (usertype.includes("basic")) {
            randomSubscription = "basic";
          } else if (usertype.includes("super")) {
            randomSubscription = "super";
          } else if (usertype.includes("premium")) {
            randomSubscription = "premium";
          }
  ...
  const handlePayment = async () => {
    ...
    setLoading(true);
    try {
      // continue to payment gateway payStack, flutterwave, etc.
      let paymentUrl = `${process.env.REACT_APP_Back_end_api_root}/initatePayment`;
      let paymentComfirmUrl = `${process.env.REACT_APP_Back_end_api_root}/confirmPayment`;
      let paymentData = {
        amount: calculatePrice() * 100, // in kobo
        email: formData.email,
        customerName: formData.fullName,
        customerPhone: formData.phone,
        machineModel: selectedProduct.title,
      ...
      let { accessCode } = await response.json();
      popup.resumeTransaction(accessCode, {
        onCancel: function () {
          setLoading(false);
        },
        onSuccess: async function (transaction) {
          // confirm transaction on the backend and show success message
          let payload = {
            reference: transaction.reference,
            shippingAddress: ` ${formData.address}, ${formData.city}, ${formData.state}, ${formData.country}, ${formData.postalCode}`,
            deviceImage: selectedProduct.image,
            ...paymentData,
          };
          let confirmResponse = await fetch(paymentComfirmUrl, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
          ...
          <div className='slhw-email-overlay'>
            ...
            <div className='slhw-email-card'>
              ...
              <form className='slhw-email-form'>
                ...
                <button disabled={loading} type='button' className='slhw-btn slhw-btn-primary' onClick={handleEmailSubmit}>
                  {loading ? (
                    loader
                  ) : (
                    <>
              ...
              <div className='slhw-benefits'>
                <div className='slhw-benefit-item'>
                  <CheckCircle size={18} />
                  <span>Account holders get up to 10% discount</span>
                </div>
                <div className='slhw-benefit-item'>
                  <CheckCircle size={18} />
                ...
                {userType === "account" && (
                  <div className='slhw-account-banner'>
                    ...
                    <div>
                      ...
                      <p>
                        You're getting a {discounts[userSubscription]}% discount with your{" "}
              ...
              <div className='slhw-payment-section'>
                <div className='slhw-payment-card'>
                  ...
                  <div className='slhw-order-summary'>
                    ...
                    <button disabled={loading} className='slhw-btn slhw-btn-primary slhw-btn-full' onClick={handlePayment}>
                      {loading ? (
                        loader
                      ) : (
                        <>
                    ...
                    <button disabled={loading} className='slhw-btn slhw-btn-secondary slhw-btn-full' onClick={() => setCheckoutStep("details")}>
                      Back to Checkout
                    </button>
                  </div>
                </div>
...
const loader = (
  <svg width='30' height='30' viewBox='0 0 100 100' xmlns='http://www.w3.org/2000/svg'>
    <circle cx='50' cy='50' r='40' fill='none' stroke='#ffffff' stroke-width='8' stroke-dasharray='251.2' stroke-dashoffset='62.8' stroke-linecap='round'>
